...
class _FollowingPageState extends State<FollowingPage> {
  ...
  Future delFollowingList(bool dontPOP) async {
    SharedPreferences? prefs = await SharedPreferences.getInstance();
    if (InVisUser == false) {
      await prefs!.setStringList('followings', []);
    } else {
      await prefs!.setStringList('Vfollowings', []);
    }
    if (dontPOP == false) {
  ...
  List<Following> FollowingL = [];
  Future getFollowingList() async {
    List<String>? followingsss = [];
    SharedPreferences? prefs = await SharedPreferences.getInstance();
    QuerySnapshot u =
        await FirebaseFirestore.instance.collection('users').get();
    if (InVisUser == false) {
      followingsss = await prefs!.getStringList('followings');
    } else {
      followingsss = await prefs!.getStringList('Vfollowings');
    }
    for (int i = 0; i < followingsss!.length; i++) {
      for (int j = 0; j < u.docs.length; j++) {
        if (u.docs[j]["token1"] == followingsss[i]) {
          FollowingL.add(Following(
              name: u.docs[j]["name"],
  ...
  Future onUnFollow(String id) async {
    SharedPreferences? prefs = await SharedPreferences.getInstance();
            ...
            if (InVisUser == false) {
              return Container(
                child: OutlinedButton(
                    onPressed: () async {
                      showToast('Unfollowing user, please wait...',
                          Toast.LENGTH_SHORT);
                      await onUnFollow(u.token1);
                      logCustomEvent(
                          widget.analytics, 'unfollow profile button tapped');
                      await delFollowingList(true);
                      showToast(
                          'User unfollowed! Please scroll down to refresh.',
                          Toast.LENGTH_SHORT);
                      Navigator.pop(context);
                    },
                    child: Icon(
